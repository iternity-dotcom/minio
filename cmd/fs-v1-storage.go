...
func (s *fsv1Storage) NSScanner(ctx context.Context, cache dataUsageCache, updates chan<- dataUsageEntry) (dataUsageCache, error) {
	defer close(updates)
	// Get bucket policy
	...
	cache, err = scanDataFolder(ctx, s.String(), cache, func(item scannerItem) (sizeSummary, error) {
		...
		sz := item.applyActions(ctx, objAPI, actionMeta{oi: oi}, &sizeSummary{})
		if sz >= 0 {
...
func (s *fsv1Storage) RenameFile(ctx context.Context, srcVolume, srcPath, dstVolume, dstPath string) (err error) {
	...
	if !HasPrefix(srcVolume, minioMetaBucket) && dstVolume == s.metaTmpBucket {
		...
		metaSrcPath := pathutil.Join(metaVolumeDir, bucketMetaPrefix, srcVolume, srcPath)
...
func (s *fsv1Storage) LastConn() time.Time {
	return time.Time{}
}
