...
func (fs *FSObjects) backgroundAppend(ctx context.Context, bucket, object, uploadID string) {
	...
	if file == nil {
		file = &fsAppendFile{
			filePath: pathJoin(fs.disk.String(), minioMetaTmpBucket, fs.fsUUID, fmt.Sprintf("%s.%s", uploadID, mustGetUUID())),
		}
...
func (fs *FSObjects) PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, r *PutObjReader, opts ObjectOptions) (pi PartInfo, e error) {
	...
	tmpPartPath := pathJoin(fs.disk.String(), minioMetaTmpBucket, fs.fsUUID, uploadID+"."+mustGetUUID()+"."+strconv.Itoa(partID))
