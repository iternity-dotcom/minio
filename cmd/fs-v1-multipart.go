...
func (fs *FSObjects) NewMultipartUpload(ctx context.Context, bucket, object string, opts ObjectOptions) (string, error) {
	...
	if !isMinioMetaBucketName(bucket) && !hasSpaceFor([]*DiskInfo{&di}, -1) {
...
func (fs *FSObjects) PutObjectPart(rctx context.Context, bucket, object, uploadID string, partID int, r *PutObjReader, opts ObjectOptions) (pi PartInfo, e error) {
	...
	if err := checkPutObjectPartArgs(rctx, bucket, object, fs); err != nil {
		return pi, toObjectErr(err, bucket)
	}

	// Write lock for this part ID.
	// Held throughout the operation.
	partIDLock := fs.NewNSLock(bucket, pathJoin(object, uploadID, strconv.Itoa(partID)))
	plkctx, err := partIDLock.GetLock(rctx, globalOperationTimeout)
	if err != nil {
		return PartInfo{}, err
	}
	pctx := plkctx.Context()
	...
	uploadIDRLock := fs.NewNSLock(bucket, pathJoin(object, uploadID))
	rlkctx, err := uploadIDRLock.GetRLock(rctx, globalOperationTimeout)
	if err != nil {
		return pi, err
	}
	rctx = rlkctx.Context()
	...
	if _, err := fs.disk.StatVol(rctx, bucket); err != nil {
		return pi, toObjectErr(err, bucket)
	}
	...
	fi, err := fs.disk.ReadVersion(rctx, minioMetaMultipartBucket, uploadIDPath, "", false)
	if err != nil {
	...
	err = fs.disk.CreateFile(pctx, fs.disk.MetaTmpBucket(), tmpPartPath, cReader.Size(), cReader)
	if err != nil {
	...
	uploadIDWLock := fs.NewNSLock(bucket, pathJoin(object, uploadID))
	...
	fi, err = fs.disk.ReadVersion(wctx, minioMetaMultipartBucket, uploadIDPath, "", false)
	if err != nil {
		if err == errFileNotFound || err == errFileAccessDenied {
			return pi, InvalidUploadID{Bucket: bucket, Object: object, UploadID: uploadID}
		}
	...
	err = fs.disk.RenameFile(wctx, fs.disk.MetaTmpBucket(), tmpPartPath, minioMetaMultipartBucket, partPath)
	if err != nil {
		if err == errFileNotFound || err == errFileAccessDenied {
			return pi, InvalidUploadID{Bucket: bucket, Object: object, UploadID: uploadID}
		}
	...
	if err = fs.disk.WriteMetadata(wctx, minioMetaMultipartBucket, uploadIDPath, fi); err != nil {
		return pi, toObjectErr(err, minioMetaMultipartBucket, uploadIDPath)
	}

	go fs.backgroundAppend(wctx, bucket, object, uploadID, fi)
...
func (fs *FSObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart, opts ObjectOptions) (ObjectInfo, error) {
	...
	uploadIDLock := fs.NewNSLock(bucket, pathJoin(object, uploadID))
	upllkctx, err := uploadIDLock.GetRLock(ctx, globalOperationTimeout)
	if err != nil {
		return ObjectInfo{}, err
	}
	defer uploadIDLock.RUnlock(upllkctx.Cancel)
	ctx = upllkctx.Context()
	...
	dstlkctx, err := destLock.GetLock(ctx, globalOperationTimeout)
	if err != nil {
		return ObjectInfo{}, err
	}
	ctx = dstlkctx.Context()
	defer destLock.Unlock(dstlkctx.Cancel)
...
func (fs *FSObjects) deleteUploadFiles(ctx context.Context, uploadID string, uploadIDPath string) {
	fs.appendFileMapMu.Lock()
	...
	fs.disk.Delete(ctx, minioMetaMultipartBucket, uploadIDPath, true)
}
